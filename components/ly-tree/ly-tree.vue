...
<script>
	...
	export default {
		...
		methods: {
			...
			/*
				说明：对树节点进行筛选操作
				参数：接收一个任意类型的参数，该参数会在 filter-node-method 中作为第一个参数
			*/
			filter(value) {
			...
			*/
			getNodePath(data) {
				...
				while (parent && parent !== this.store.root) {
			...
			*/
			...
			handleNodeExpand(e) {
				let node = e;
				// #ifndef H5
				node = e.detail.__args__[0];
				// #endif
				this.broadcast('LyTreeNode', 'tree-node-expand', node);
