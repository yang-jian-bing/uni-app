    ...
    <view>
        <view class="content">
            <view class="joiList" v-for="item in datalist" :key="item.usr_id" @click="goto(item.id)">
                ...
                <view class='weWzpad'>
                    <p class='font14'>采购标题：{{item.title}}</p>

                    <p class='font14'>创建时间：{{item.created}}</p>


                    <p class='font14'>创建人：{{item.modificator}}</p>
                </view>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onLoad() {
                getHandle("workflow/rest/v0/mobile/get.wf?page=1&limit=13&module=3&menu=1")
                    .then(res => {
                        console.log(res)
